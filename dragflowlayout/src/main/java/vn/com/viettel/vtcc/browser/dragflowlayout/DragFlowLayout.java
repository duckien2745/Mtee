...
public class DragFlowLayout extends FlowLayout implements IViewManager {
    ...
    public interface OnItemClickListener {
        ...
        boolean performClick(DragFlowLayout dragFlowLayout, View child,
                             MotionEvent event, int dragState, int index);
    }
    /**
...
*/
    ...
    private class GestureListenerImpl extends GestureDetector.SimpleOnGestureListener{
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            ...
            final List<Item> mItems = DragFlowLayout.InternalItemHelper.mItems;

            DragFlowLayout.Item item ;
            index = INVALID_INDXE;
            for(int i = 0, size = mItems.size(); i<size ; i++){
                item =  mItems.get(i);
                if(item.view == mTouchChild){
                    index = item.index ;
                    break;
                }
            }
            boolean performed = mClickListener.performClick(DragFlowLayout.this, mTouchChild, e , mDragState,index);
            if(performed){
                playSoundEffect(SoundEffectConstants.CLICK);
            }else if (mReDrag) {
                checkForDrag(200, true);
            }
            return performed;
        }
        @Override
